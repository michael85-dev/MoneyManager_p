...
<div>
	<c:choose>
		<c:when test="">
			<form>
				<select id="atod1">
					<c:forEach items="${a3}" var="3">
						<option value="${3.ai_name}">${3.ai_name}</option>
					</c:forEach>
				</select>
			...
			<form>
				<select id="dtoa1">
					<c:forEach items="${d1}" var="s">
						<option value="${1.di_name}">${1.di_name}</option>
					</c:forEach>
				</select>
				<input type="text" id="send">
				<input type="text" name="di_pAsset">
			</form>
		</c:when>
		<c:otherwise>
			<form>
				<select id="dtoa2">
					<c:forEach items="${d1}" var="1">
						<option value="${1.di_name}">${1.di_name}</option>
					</c:forEach>
				</select>
				<input type="text" id="send">
